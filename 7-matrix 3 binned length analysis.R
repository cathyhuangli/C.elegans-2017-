...
all_len=sort(unique(cluster2.rle$lengths))
...
extraction <- lapply(video_paths, function(x) {
  ...
  for( i in row ){
    for( j in c(1,2,3) ){
      ...
      #return index of j in the column values=index in matrix
      mymatrix[i,j]=value
    }
  }
  mymatrix[1,4]=sum(df$value==1 & (df$length>=4 & df$length<=8))
  mymatrix[1,5]=sum(df$value==1 & (df$length>=9 & df$length<=16))
  mymatrix[1,6]=sum(df$value==1 & (df$length>=17 & df$length<=32))
  mymatrix[1,7]=sum(df$value==1 & df$length>=33)
  mymatrix[2,4]=sum(df$value==2 & (df$length>=4 & df$length<=8))
  mymatrix[2,5]=sum(df$value==2 & (df$length>=9 & df$length<=16))
  mymatrix[2,6]=sum(df$value==2 & (df$length>=17 & df$length<=32))
  mymatrix[2,7]=sum(df$value==2 & df$length>=33)
...
write.csv(mydf,file='rle_5type_11extraction_binnedindex.csv')

########################################################
######### calculate correlation & kmeans all features:

mydf=rle_5type_11extraction_binnedindex[,-c(1,2)]
#cor(mydf)
...
df.f.labels = cbind(rle_5type_11extraction_binnedindex,cluster=factor(km.rle$cluster))
...
#write.csv(df.f.labels,file="raw-length-binnedindex matrix 11 descriptors.csv")
...
mydf=rbindlist(extraction)
write.csv(mydf,file='rle_5type_11extraction_binnedavg.csv')

########################################################
######### calculate correlation & kmeans all features:

mydf=rle_5type_11extraction_binnedavg[,-c(1,2)]
#cor(mydf)
########## K-MEANS CLUSTERING: K=5 TYPES
set.seed(1234)
rle.norm<-normalized(mydf)
k.f = 5 # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
km.rle<-kmeans(rle.norm,k.f,iter.max = 1000,nstart = 25)
df.f.labels = cbind(rle_5type_11extraction_binnedavg,cluster=factor(km.rle$cluster))
df.f.labels$v_type=gsub("_f[0-9]*", "_f", gsub("_nf[0-9]*", "_nf",gsub("_nnf[0-9]*", "_nnf", df.f.labels$videoname)))
df.f.labels$v_name=gsub("_f[0-9]*", "", gsub("_nf[0-9]*", "",gsub("_nnf[0-9]*", "", df.f.labels$videoname)))
# no need: df.f.labels <- as.data.frame(lapply(df.f.labels, function(x){replace(x, x=='N2_nnf','N2_nf')}))
...
mydf=rle_5type_11extraction_binnedavg[,-c(1,2)]
#cor(mydf)
cor(mydf[,c('RP','LRHGE','SRHGE')])
mydf=mydf[,c('RP','LRHGE','SRHGE')]
########## K-MEANS CLUSTERING: K=5 TYPES
