...
write.csv(mydf,file='rle_5type_11extraction_logofj.csv')

########################################################
######### calculate correlation & kmeans all features:

mydf=rle_5type_11extraction_logofj[,-c(1,2)]
cor(mydf)
...
df.f.labels = cbind(rle_5type_11extraction_logofj,cluster=factor(km.rle$cluster))
...
mydf=rle_5type_11extraction_logofj[,-c(1,2)]
#cor(mydf)
cor(mydf[,c('RP','SRE','GLN')])
mydf=mydf[,c('RP','SRE','GLN')]
########## K-MEANS CLUSTERING: K=5 TYPES
set.seed(1234)
rle.norm<-normalized(mydf)
k.f = 5 # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
