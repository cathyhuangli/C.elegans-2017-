...
for (i in 2:5){
  ...
  report(df.f.labels[,c(2,3,6)], "cluster")
  outfile=paste("allofdata.assigned", i, ".csv",sep = "")
  write.csv(df.f.labels,file=outfile)
}

k.f =2# i # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
km.allofdata<-kmeans(allofdata.norm[,1:3],k.f,iter.max = 1000,nstart = 25)
df.f.labels = cbind(allofdata,cluster=factor(km.allofdata$cluster))
df=df.f.labels
# k.f.=2
summary(df[df$cluster==1,c(2,3,6)])
summary(df[df$cluster==2,c(2,3,6)])

k.f =3# i # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
km.allofdata<-kmeans(allofdata.norm[,1:3],k.f,iter.max = 1000,nstart = 25)
df.f.labels = cbind(allofdata,cluster=factor(km.allofdata$cluster))
df=df.f.labels
# k.f.=3
summary(df[df$cluster==1,c(2,3,6)])
summary(df[df$cluster==2,c(2,3,6)])
summary(df[df$cluster==3,c(2,3,6)])

k.f =4# i # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
km.allofdata<-kmeans(allofdata.norm[,1:3],k.f,iter.max = 1000,nstart = 25)
df.f.labels = cbind(allofdata,cluster=factor(km.allofdata$cluster))
df=df.f.labels
# k.f.=4
...
summary(df[df$cluster==2,c(2,3,6)])
summary(df[df$cluster==3,c(2,3,6)])
summary(df[df$cluster==4,c(2,3,6)])


k.f =5# i # <--------------------||                  # Change this for desired number of cluster if you forget to input in the beginning
km.allofdata<-kmeans(allofdata.norm[,1:3],k.f,iter.max = 1000,nstart = 25)
df.f.labels = cbind(allofdata,cluster=factor(km.allofdata$cluster))
df=df.f.labels
# k.f.=5
...
summary(df[df$cluster==3,c(2,3,6)])
summary(df[df$cluster==4,c(2,3,6)])
summary(df[df$cluster==5,c(2,3,6)])


##############summary of the 11 descrtipors#############

summary(descriptors.summary[,3:13])
